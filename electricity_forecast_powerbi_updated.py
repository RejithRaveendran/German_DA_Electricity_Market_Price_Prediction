...
def prev_day_actual_vs_pred(prev_local_date):
    ...
    # Quick fix: Use tolerance-based logic for in_band
tolerance = 10  # EUR/MWh
comp["in_band"] = (comp["abs_error"] <= tolerance).astype(int)
...
# Debug: Check p10 and p90 spread
p10 = prev_comp_df["pred_p10_eur_per_mwh"]
p90 = prev_comp_df["pred_p90_eur_per_mwh"]
print("
🔍 Debug: Quantile Prediction Summary")
print(f"p10 → min: {p10.min():.2f}, max: {p10.max():.2f}, mean: {p10.mean():.2f}")
print(f"p90 → min: {p90.min():.2f}, max: {p90.max():.2f}, mean: {p90.mean():.2f}")
print(f"Average band width (p90 - p10): {(p90 - p10).mean():.2f}")
if (p90 - p10).mean() < 1:
    print("⚠ Warning: Prediction band is extremely narrow. Quantile models may be collapsing.")
else:
    print("✅ Band width looks reasonable.")
